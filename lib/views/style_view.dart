      ...
      children: [
        SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              Center(
                child: Text(txt, style: const TextStyle(color: Colors.white, fontSize: 20, fontStyle: FontStyle.italic),),
              ),
              Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children:[ Row(
...
class _StyleViewState extends State<StyleView> {
  ...
  Future<void> _loadColor(Color color) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
  ...
  Future<void> _loadPieceStyle(int index) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    switch(index){
      case 0: {
          ...
          child: Column(
            children: [
              const TabBar(
                isScrollable: false,
                tabs: [
                  Tab(child: Text('Board color', style: TextStyle(color: Colors.white),),),
                  Tab(child: Text('Pieces style', style: TextStyle(color: Colors.white),),),
                ],
              ),
              SizedBox(
                          ...
                          children: colors.map((List<Color> colorPair) {
                            return GestureDetector(
                              onTap: () async {
                                ...
                                await _loadColor(colorPair[0]);
                              },
                              child: Container(
                                decoration: BoxDecoration(
                                  border: Border.all(
                                    color: selectedColor == colorPair[0] ? const Color.fromRGBO(183, 0, 0, 1) : Colors.black,
                                    width: 4,
                                  ),
                                ),
                          ...
                          children: pieces.map((List<SvgPicture> style) {
                            ...
                            return GestureDetector(
                              onTap: () async {
                                ...
                                await _loadPieceStyle(index);
                              },
                              child: Container(
                                decoration: BoxDecoration(
                                  border: Border.all(
                                    color: selectedStyle == index ? const Color.fromRGBO(183, 0, 0, 1) : Colors.transparent,
                                    width: 4,
                                  ),
                                ),
